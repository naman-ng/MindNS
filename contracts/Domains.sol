...
contract Domains is ERC721URIStorage {
    ...
    function register(string calldata name) public payable {
        require(domains[name] == address(0), "wtf dude");
        ...
        string memory _name = string(abi.encodePacked(name, ".", domainName));
        ...
        console.log("The domain is %s and the tokenId is %s", _name, currentId);

        string memory json = Base64.encode(
            abi.encodePacked(
                '{"name": "',
                _name,
                '", "description": "A domain on the Ethereum mind name service", "image": "data:image/svg+xml;base64,',
    ...
    function setData(string calldata _name, string calldata _data) public {
        require(msg.sender == domains[_name], "Hahahahahha");
        data[_name] = _data;
    }
    ...
    function getData(string calldata _name)
        public
        view
